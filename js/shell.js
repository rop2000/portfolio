...
class Shell {
  constructor (term, commands) {
    ...
    localStorage.directory = 'rohanparikh'
    localStorage.history = JSON.stringify('') //command history json 
  ...
  setupListeners (term) {
    ...
    term.addEventListener('keyup', (evt) => {
      ...
      if ([keyUp, keyDown].includes(key)) {
        ...
        history = history ? Object.values(JSON.parse(history)) : [] //cancer ternary
        if (key === keyUp) {
          if (localStorage.historyIndex >= 0) {
            if (!localStorage.inHistory) {
              localStorage.inHistory = true
            } else {
              // Stops dumb command from looping
          ...
          if (localStorage.inHistory && localStorage.historyIndex < history.length) {
            let ret
            if (localStorage.historyIndex > 0) {                                    //stolen function, i'm not big brain enough for this
    ...
    term.addEventListener('keypress', (evt) => {
      ...
      if (evt.keyCode === 13) {
        ...
        if (cmd === 'clear') {
          ...
          this.term.innerHTML += cmd ? 'Error: ur using a fake command' : '';
          this.resetPrompt(term, prompt)
        }
        evt.preventDefault()
      }
  ...
  clearConsole () {
    $('#terminal').html(
      `<p class="hidden">
        <span class="prompt">
          <span class="root">rohanparikh</span>
